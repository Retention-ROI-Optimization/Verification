...
## 4-5. 공개 데이터(dunnhumby Complete Journey)로 raw_grid 만들기

기존 합성 시뮬레이터 대신 공개 데이터를 사용하려면 먼저 아래 변환 스크립트로
`artifacts/raw_grid/seed_XX/`에 실험 베드가 기대하는 CSV들을 생성합니다.

```bash
python scripts/prepare_dunnhumby_complete_journey.py   --zip-path /path/to/dunnhumby_The-Complete-Journey.zip   --project-root .   --seeds 141,142,143
```
...
그 다음에는 기존과 동일하게 `run-paper`를 실행하면 됩니다. `prepare_simulation_grid`
단계는 필요한 raw grid가 이미 존재하면 이를 재사용하고 합성 데이터를 다시 만들지 않습니다.

```bash
python main.py   --mode run-paper   --project-root .   --seeds 141,142,143   --scenario-families complaint-heavy,promotion-heavy,dormancy-heavy,seasonal-shift   --latencies 0,1,3,7   --budgets 220000,390000,560000   --burn-in-weeks 12   --training-landmarks 12   --decision-week-limit 16   --bootstrap-iterations 300
```
